...
export default function FormCanvas({ form, selectedId, onSelect, onChange, onAdd, onMove, onDuplicate, onDelete }: Props) {
    ...
    const drop = (event: React.DragEvent, index: number) => {
        ...
        else if (id) onMove(id, Math.min(index, form.fields.length - 1));
    };
    return <main className="studio-canvas" aria-label="Form canvas" onDragEnd={() => setDropIndex(null)}>
        <div className="studio-canvas-inner">
            ...
            {form.fields.map((field, index) => <FieldCard key={field.id} field={field} index={index} count={form.fields.length} selected={field.id === selectedId} dropTarget={dropIndex === index} onSelect={() => onSelect(field.id)} onMove={(target) => onMove(field.id, target)} onDuplicate={() => onDuplicate(field.id)} onDelete={() => onDelete(field.id)} onDragOver={(event) => { event.preventDefault(); setDropIndex(index); }} onDrop={(event) => drop(event, index)} />)}
            <div className="studio-quick-add" onDragOver={(event) => { event.preventDefault(); setDropIndex(form.fields.length); }} onDrop={(event) => drop(event, form.fields.length)} style={dropIndex === form.fields.length ? { borderColor: "#047857", background: "#d1fae5" } : undefined}>
