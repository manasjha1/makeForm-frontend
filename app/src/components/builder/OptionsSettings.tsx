...
export default function OptionsSettings({ field, onChange }: SettingsProps) {
    ...
    return <div><h3 className="studio-label">Options Editor</h3><p className="studio-muted mt-1">Add, rename, or reorder the available choices.</p>
        {options.map((option, index) => <div className="studio-option-row" key={index}>
            <input className="studio-control min-w-0" aria-label={`Option ${index + 1}`} value={option} onChange={(event) => update(options.map((value, position) => position === index ? event.target.value : value))} />
        ...
        <button className="studio-button mt-3 w-full" onClick={() => update([...options, `Option ${options.length + 1}`])}><Plus size={13} />Add Option</button>
