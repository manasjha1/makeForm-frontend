...
export default function FieldsPalette({ onAdd }: { onAdd: (type: FieldType) => void }) {
    ...
    return <aside className="studio-panel studio-palette" aria-label="Fields palette">
        <div className="studio-panel-header">
            ...
            <div className="studio-search"><Search size={14} /><input className="studio-control" aria-label="Search field types" placeholder="Search field types..." value={search} onChange={(event) => setSearch(event.target.value)} /></div>
        ...
        </button>)}{!fields.length && <p className="studio-empty">No matching fields. Try another search.</p>}</div>
